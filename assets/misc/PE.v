...
(* 239: Have an exercise or two for the students to try. *)

(* ================================================================= *)
(** ** CS239 supplement: [aexp] / [bexp] AST helpers and extra exercises *)

Inductive aexp_ast : Type :=
  | N (n : nat)
  | I (x : string)
  | P (a1 a2 : aexp_ast)
  | Sub (a1 a2 : aexp_ast)
...
Fixpoint aexp_to_ast (a : aexp) : aexp_ast :=
  match a with
  | ANum n => N n
  | AId x => I x
  | APlus a1 a2 => P (aexp_to_ast a1) (aexp_to_ast a2)
  | AMinus a1 a2 => Sub (aexp_to_ast a1) (aexp_to_ast a2)
  | AMult a1 a2 => M (aexp_to_ast a1) (aexp_to_ast a2)
  end.

Compute (aexp_to_ast <{X + 1 + Y}>).

(** Sean: Exercise for students *)
(** Effect of Associativity *)
Compute (aexp_to_ast <{X - 1 * Y}>).
Example excercise_aexp_1 :
  pe_aexp [(X,2)] <{X - 1 * Y}> = <{239}>.
Proof. reflexivity. Qed.

(** Effect of Precedence *)
Compute (aexp_to_ast <{X * 1 - Y}>).
Example excercise_aexp_2 :
  pe_aexp [(Y,2)] <{X - 1 * Y}> = <{239}>.
Proof. reflexivity. Qed.

(** Misc Example *)
Compute (aexp_to_ast <{Y * 1 - X * (1 - X * 1 - X) }>).
Example excercise_aexp_3 :
  pe_aexp [(X, 2);(Y, 1)] <{Y * 1 - X * (1 - X * 1 - X)}> = <{239}>.
Proof. reflexivity. Qed.
...
(* 239: Again, an exercise or two will help. *)  

Inductive bexp_ast : Type :=
  | Tt
  | Ff
  | Eq (a1 a2 : aexp_ast)
  | Neq (a1 a2 : aexp_ast)
...
Fixpoint bexp_to_ast (b : bexp) : bexp_ast :=
  match b with
  | BTrue => Tt
  | BFalse => Ff
  | BEq a1 a2 => Eq (aexp_to_ast a1) (aexp_to_ast a2)
  | BNeq a1 a2 => Neq (aexp_to_ast a1) (aexp_to_ast a2)
  | BLe a1 a2 => Le (aexp_to_ast a1) (aexp_to_ast a2)
  | BGt a1 a2 => Gt (aexp_to_ast a1) (aexp_to_ast a2)
  | BNot b1 => Not (bexp_to_ast b1)
  | BAnd b1 b2 => And (bexp_to_ast b1) (bexp_to_ast b2)
  end.

Compute (bexp_to_ast <{ ~(X <= 3) }>).
Example test_pe_bexp1:
  pe_bexp [(X,3)] <{~(X <= 3)}>
  = <{ false }>.
Proof. reflexivity. Qed.

Compute (bexp_to_ast <{ ~(X <= (X + 1))  }>).
...
(** Sean: Exercise for students *)
(** pe_aexp being used in pe_bexp *)
Example exercise_bexp_1 :
  pe_bexp [(X,2)] <{~((X+Y) <= X)}> = <{ true }>.
Proof. reflexivity. Qed.

(** Statically evaluating entire bexp *)
Example exercise_bexp_2 :
  pe_bexp [(X,2);(Y,0)] <{~((X+Y) <= X)}> = <{ true }>.
Proof. reflexivity. Qed.

(** Unable to statically evaluate entire bexp *)
Example exercise_bexp_3 :
  pe_bexp [(X,2)] <{~((X+Y) > (X+Y))}> = <{ true }>.
Proof. reflexivity. Qed.
...
(* 239: Useful to go down and look at the definition of partial evaluation for assignments now, before moving on to conditionals. Can probably try an exercise or two as well. *)

(** Sean: jump to 'assignment-example'*)
...
(** Trickier than assignments to partially evaluate is the
    ...
    (* 239: Note that X <= Y is supposed to be X = Y *)
...
(* 239: Sean/Brian can come up with some examples and ask people to figure out what the results will be. *)
...
(** Sean: Helper for students *)

Fixpoint pe_eval_loopfree (c : com) (pe_st : pe_state)
  : option (com * pe_state) :=
  match c with
  | CSkip => Some (CSkip, pe_st)
  | CAsgn l a =>
      let a' := pe_aexp pe_st a in
      match a' with
      | ANum n => Some (CSkip, pe_add pe_st l n)
      | _ => Some (CAsgn l a', pe_remove pe_st l)
      end
  | CSeq c1 c2 =>
      match pe_eval_loopfree c1 pe_st with
      | None => None
      | Some (c1', pe_st') =>
          match pe_eval_loopfree c2 pe_st' with
          | None => None
          | Some (c2', pe_st'') =>
              Some (CSeq c1' c2', pe_st'')
          end
      end
  | CIf b c1 c2 =>
      let b' := pe_bexp pe_st b in
      match b' with
      | BTrue => pe_eval_loopfree c1 pe_st
      | BFalse => pe_eval_loopfree c2 pe_st
      | _ =>
          match pe_eval_loopfree c1 pe_st, pe_eval_loopfree c2 pe_st with
          | Some (c1', pe_st1), Some (c2', pe_st2) =>
              let cmp := pe_compare pe_st1 pe_st2 in
              Some (CIf b' (CSeq c1' (assign pe_st1 cmp))
                        (CSeq c2' (assign pe_st2 cmp)),
                    pe_removes pe_st1 cmp)
          | _, _ => None
          end
      end
  | CWhile _ _ => None
...
(** Sean: Exercise for students assignment-example *)
(** Static Assignment *)
Example pe_eval_example0 :
  pe_eval_loopfree <{X := 3 ; Y := Z * (X + X)}> [(Z,1)] = Some (<{skip}>, []).
Proof. reflexivity. Qed.

(** Dynamic Assignment *)
Example pe_eval_example1 :
  pe_eval_loopfree <{X := 3 ; Y := Z * (X + X); X := 4 + Y}> [] = Some (<{skip}>, []).
Proof. reflexivity. Qed.

(** If-True *)
Example pe_eval_example2 :
  pe_eval_loopfree <{X := 3 ; if X <= 4 then X := 4 else skip end}> [] = Some (<{skip}>, []).
Proof. reflexivity. Qed.

(** If *)
Example pe_eval_example3 :
  pe_eval_loopfree
    <{X := 1;
      if Y <= X then
        Z := 1; Y := 2; X := Y + Z
...
(* 239: Auxiliary definition used to prove correctness: It means that st takes c' to st' and applying the partial state pe_st' to st' yields st''. So nothing interesting yet. *)  
...
(* 239: Translation: If we partially evaluate c and pe_st to c' and pe_st', then for any state st, applying pe_st to st and then executing c yields the same final state as executing c' and then applying pe_st' to the resulting state. Best shown as a commuting diagram! So in particular for the case when pe_st is empty we get that the residual program c' and partial state pe_st' are consistent with all terminating executions of the original program c faithfully. *)
...
(* 239: This is the converse of the above -- it says that every execution of the residual program has a counterpart in the original program. *)
...
Inductive pe_com : com -> pe_state -> com -> pe_state -> com -> Prop :=
  ...
  | PE_WhileFalse : forall pe_st b1 c1,
      (* 239: the loop is never entered *)
      pe_bexp pe_st b1 = <{ false }> ->
      <{while b1 do c1 end}> / pe_st ==> <{skip}> / pe_st / <{skip}>
  | PE_WhileTrue : forall pe_st pe_st' pe_st'' b1 c1 c1' c2' c2'',
      (* 239: the loop is entered at least once; unroll it *)
      pe_bexp pe_st b1 = <{ true }> ->
      c1 / pe_st ==> c1' / pe_st' / <{skip}> ->
      <{while b1 do c1 end}> / pe_st' ==> c2' / pe_st'' / c2'' ->
      (* 239: the following condition just ensures that partial evaluation did at least one useful thing *)
      pe_compare pe_st pe_st'' <> [] ->
      <{while b1 do c1 end}> / pe_st ==> <{c1';c2'}> / pe_st'' / c2''
  | PE_While : forall pe_st pe_st' pe_st'' b1 c1 c1' c2' c2'',
      (* 239: conditional version of the above, when we don't know if the loop is entered or not *)
      pe_bexp pe_st b1 <> <{ false }> ->
      pe_bexp pe_st b1 <> <{ true }> ->
      c1 / pe_st ==> c1' / pe_st' / <{skip}> ->
      <{while b1 do c1 end}> / pe_st' ==> c2' / pe_st'' / c2'' ->
  ...
  | PE_WhileFixedEnd : forall pe_st b1 c1,
      (* 239: base case for a residual loop to execute after unrolling (I think) *)
      pe_bexp pe_st b1 <> <{ false }> ->
      <{while b1 do c1 end}> / pe_st ==> <{skip}> / pe_st / <{while b1 do c1 end}>
  | PE_WhileFixedLoop : forall pe_st pe_st' pe_st'' b1 c1 c1' c2',
      (* 239: an infinite loop -- identified since the loop guard evaluates to true initially and no relevant variables change (or else we'd see that in the result of partial evaluation of the loop) *)
      pe_bexp pe_st b1 = <{ true }> ->
      c1 / pe_st ==> c1' / pe_st' / <{skip}> ->
      <{while b1 do c1 end}> / pe_st'
  ...
  | PE_WhileFixed : forall pe_st pe_st' pe_st'' b1 c1 c1' c2',
      (* 239: Since pe_st and pe_st'' are the same, we know that the partial evaluation is sound for any arbitrary iteration of the loop so can just update the loop directly without unrolling *)
      pe_bexp pe_st b1 <> <{ false }> ->
      pe_bexp pe_st b1 <> <{ true }> ->
      c1 / pe_st ==> c1' / pe_st' / <{skip}> ->
      <{while b1 do c1 end}> / pe_st'
...
(* 239: Sean/Brian can create a few more examples as exercises. It would also be nice to have a few where the last command is not simply skip. *)
...
(* 239: Analogous to pe_ceval from earlier. It means that c' takes st to st', applying pe_st' to st' and running c'' from there yields st'', with a loop unrolling count less than n *)
...
(* 239: These are analogous to the earlier correctness theorems. *)  
...
(* 239: Recursively look up the block associated with a label, execute it, look up the next block from the resulting label, etc. *)
...
(* 239: It may be best to skip this part, since it's not that deeply explained and it's unclear how the PE here relates to the PE that we did on programs above. *)
